...
def _fdr_bh(pvals: np.ndarray) -> np.ndarray:
    ...
    for i, idx in enumerate(order, start=1):
        q = p[idx] * n / i
        cummin = min(cummin, q)  # força monotonicidade não crescente ao voltar
        ranked[idx] = cummin
    return np.clip(ranked, 0, 1)

def mann_whitney_screen_csv(caminho_arquivo):
    # Carregar o dataframe
    df = pd.read_csv(caminho_arquivo)  # Substitua pelo caminho do seu arquivo
    features = df.columns[1:-1]  # Exclui a primeira coluna (sequência) e a última coluna (label)
